...
def generate_context(
    doc: DocxTemplate, template_info: dict, images: list[Path]
) -> dict:
    ...
    for i, image_path in enumerate(images, start=1):
        ...
        final_image_path = (
            correct_orientation(image_path, orientation) if orientation else image_path
        )

        with PILImage.open(final_image_path) as img:
            if img.width >= img.height:
                context[f"image{i}"] = InlineImage(
                    doc, str(final_image_path), width=template_info["image_width"]
                )
            else:
                context[f"image{i}"] = InlineImage(
        ...
        if iso and f_number and exposure:
            context[f"camera{i}"] = (
                f"ISO {iso}, f{f_number}, {format_exposure(exposure)}"
            )
...
def generate_story(
    template_info: dict, images: list[Path], output_dir: Path, convert_to_pdf=True
) -> Path:
